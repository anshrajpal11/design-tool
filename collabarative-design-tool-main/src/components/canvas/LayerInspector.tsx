...
const LayerInspector = () => {
  ...
  const setFill = useMutation(
    ({ storage }, hex: string) => {
      const selection =
        (self && self.presence && self.presence.selection) || [];
  ...
  const setSize = useMutation(
    ({ storage }, size: { w: number; h: number }) => {
      const selection =
        (self && self.presence && self.presence.selection) || [];
      if (selection.length === 0) return;
      const id = selection[0] as string;
  ...
  const setBorderRadius = useMutation(
    ({ storage }, radius: number) => {
      const selection =
        (self && self.presence && self.presence.selection) || [];
      if (selection.length === 0) return;
      const id = selection[0] as string;
  ...
  React.useEffect(() => {
    if (
      ...
      self.presence.selection.length === 0
    )
      return;
    ...
    <div className="flex h-full flex-col gap-4 overflow-y-auto p-4 text-white">
      ...
      <div className="rounded-lg bg-gray-800/50 p-3">
        ...
        <input
          ...
          className="h-10 w-full cursor-pointer rounded border-2 border-gray-600"
        />
      </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                type="number"
                className="w-24 rounded border border-gray-600 bg-gray-700 px-3 py-1 text-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                onBlur={(e) =>
                  setSize({ w: Number(e.target.value), h: height })
                }
              />
      ...
      {selectedLayer.type === LayerType.Text && (
        <div className="rounded-lg bg-gray-800/50 p-3">
          ...
          <select
            ...
            className="mb-3 w-full rounded border border-gray-600 bg-gray-700 px-3 py-1 text-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
          >
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  type="number"
                  className="w-20 rounded border border-gray-600 bg-gray-700 px-3 py-1 text-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
